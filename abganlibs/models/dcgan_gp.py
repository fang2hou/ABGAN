...
class DCGAN_D(nn.Module):
    def __init__(self, isize, nz, nc, ndf, ngpu, n_extra_layers=0):
        ...
        self.ngpu = ngpu
        assert isize % 4 == 0, "isize has to be a multiple of 16"
        ...
        main.add_module('final_{0}-{1}_conv'.format(cndf, 1),
                        nn.Conv2d(cndf, 1, 3, 1, 0, bias=False))
        self.main = main
    ...
    def __init__(self, isize, nz, nc, ngf, ngpu, n_extra_layers=0):
        ...
        while csize < isize // 2:
            main.add_module('pyramid_{0}-{1}_convt'.format(cngf, cngf // 2),
                            nn.ConvTranspose2d(cngf, cngf // 2, 3, 2, 1, bias=False))
        ...
        for t in range(n_extra_layers):
            main.add_module('extra-layers-{0}_{1}_conv'.format(t, cngf),
                            nn.Conv2d(cngf, cngf, 4, 1, 1, bias=False))
        ...
        main.add_module('final_{0}-{1}_convt'.format(cngf, nc),
                        nn.ConvTranspose2d(cngf, nc, 3, 2, 1, bias=False))
