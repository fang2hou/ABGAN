...
level_width_min = -3.5
level_width_max = 8.5
level_height_min = -3.0         # only used by platforms, ground structures use absolute_ground to determine their lowest point
level_height_max = 5.0
...
number_platforms = 3            # number of platforms (reduced automatically if not enough space)
...
while (checker != ""):
    if checker == "\n":
        ...
        for current_level in range(number_levels):
            ...
            if (current_level+finished_levels) < 10:
                level_name = "0"+str(current_level+finished_levels)
            else:
                level_name = str(current_level+finished_levels)
